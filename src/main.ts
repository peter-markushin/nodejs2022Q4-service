...
async function bootstrap() {
  ...
  const orderedLogLevels: LogLevel[] = [
    'error',
    'warn',
    'log',
    'verbose',
  ...
  logService.setLogLevels(
    orderedLogLevels.slice(
      0,
      orderedLogLevels.indexOf(env.LOG_LEVEL as LogLevel) + 1,
    ),
  );
  ...
  process.on('uncaughtException', (error, origin) => {
    logService.error(
      `Uncaught process exception "${error.message}" at ${origin}`,
    );
